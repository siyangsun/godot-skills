...
- **Primary:** Read `skills/animation-system/SKILL.md` for AnimationPlayer, AnimationTree, blend spaces, animation state machines, IKModifier3D family, BoneConstraint3D, retargeting, and code-driven `Tween` motion
...
- For UI motion via Control nodes (use `godot-game-dev` with `godot-ui`)
